...
static void add_symbol_value(ListNode * const symbol_map_head,
    ...
    if (number_of_symbol_names == 1) {
        // if the last value in the list has a refcount < 0 that means it is
        // a WILL_RETURN_ONCE (at least) or WILL_RETURN_ALWAYS value and so
        // adding another value beyond it will not change the value returned to
        // the caller by get_symbol_value(), which is correct because that
        // value should "always" be returned, i.e. not affected by any other
        // attempts to define a different value to return. However, if the
        // caller is trying to define a new value to "always" return permit
        // the replacement of the current "always" value, if one exists.
        ListNode *list_head = &target_map_value->symbol_values_list_head;
        ListNode *tail_node = list_head->prev;
        if (refcount < 0 && tail_node->refcount < 0) {
            // replace the value in the current "always" item
            // TO DO: should the old value be free()'d?
            tail_node->value = value;
        } else {
            list_add_value(list_head, value, refcount);
        }
    } else {
    ...
    if (rc) {
        ...
        if (cmp == 0) {
            ...
            cm_print_error(SOURCE_LOCATION_FORMAT
                           ...
                           function);
            // // XIMON
            // ListNode *head = &global_call_ordering_head;
            // ListNode *current = NULL;
            // ListNode *next = NULL;
            // FuncOrderingValue *call = NULL;
            // for (current  = head->next, next = current->next;
            //      current != head;
            //      current  = next, next = current->next) {
            //     call = (FuncOrderingValue *)current->value;
            //     cm_print_error("XIMON: %s\n", call->function);
            // }
            exit_test(1);
        }
    } else {
